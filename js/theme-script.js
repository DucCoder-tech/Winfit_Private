/* UItoTop jQuery Plugin | Matt Varone | https://www.mattvarone.com/web-design/uitotop-jquery-plugin */ ! function(n) {
...
(function($) {
	...
	CherryJsCore.theme_script = {
		...
		swiper_carousel_init: function(self) {
			...
			jQuery('.fitspot-carousel').each(function() {
				var swiper = null,
		...
		stick_init: function(self) {
			...
			$(window).load(function() {
				...
				if (!$.isFunction(jQuery.fn.stickUp) || undefined === window.fitspot.stickUp || !$navbar.length) {
					return !1;
				}
		...
		isMegaMenuEnabled: function() {
			...
			if (undefined === window.fitspot.megaMenu) {
				return false;
			}

			if (true === window.fitspot.megaMenu.isActive && 'main' == window.fitspot.megaMenu.location) {
				return true;
			}
		...
		main_menu: function(self, $mainNavigation) {
			...
			var transitionend = 'transitionend oTransitionEnd webkitTransitionEnd',
				...
				subMenu,
				index = -1,
				$layout = window.fitspot.labels.header_layout;

			if (hasprop.call(window, 'fitspot') &&
				hasprop.call(window.fitspot, 'more_button_options') &&
				hasprop.call(window.fitspot.more_button_options, 'more_button_type')) {
				switch (window.fitspot.more_button_options.more_button_type) {
					case 'image':
						imgurl = window.fitspot.more_button_options.more_button_image_url;
						if (window.fitspot.more_button_options.retina_more_button_image_url) {
							srcset = ' srcset="' + window.fitspot.more_button_options.retina_more_button_image_url + ' 2x"';
						}
						moreMenuContent = '<img src="' + imgurl + '"' + srcset + ' alt="' + moreMenuContent + '">';
						hasimg = true;
						break;
					case 'icon':
						moreMenuContent = '<i class="fa ' + window.fitspot.more_button_options.more_button_icon + '"></i>';
						hasicon = true;
						break;
					case 'text':
					default:
						moreMenuContent = window.fitspot.more_button_options.more_button_text || moreMenuContent;
						hasimg = false;
						hasicon = false;
						break;
				}
		...
		page_preloader_init: function(self) {
			...
			if ($('.page-preloader-cover')[0]) {
				...
				function collectCriticalImageUrls() {
					// Collect from img tags - only above the fold (reduced to 1x viewport for speed)
					$('img').each(function() {
						var $img = $(this);
						...
						// Reduced to 1x viewport (was 1.5x) - only truly visible images
						var isAboveFold = offset && offset.top < viewportHeight;
						
						// Only include critical images that are visible
						if (!isLazy && isAboveFold) {
							var src = $img.attr('src');
							if (src && !src.startsWith('data:')) {
								// Limit to first 10 images max
								if (criticalImageUrls.size < 10) {
					...
					// Collect only critical background images (header only - most important)
							...
							if (bg && bg !== 'none') {
								...
								if (match && match[1]) {
									...
									if (!url.startsWith('data:')) {
										url = normalizeUrl(url);
										if (url && criticalImageUrls.size < 10) {
					...
					// Add preloaded critical images from HTML (these are highest priority)
					$('link[rel="preload"][as="image"]').each(function() {
						var href = $(this).attr('href');
						if (href && !href.startsWith('data:')) {
							// Preloaded images get priority - add them first
							var normalizedUrl = normalizeUrl(href);
							// Remove from set if exists, then add to front
							criticalImageUrls.delete(normalizedUrl);
							criticalImageUrls.add(normalizedUrl);
						}
					});
					
					// Debug: Log how many critical images found
					console.log('Preloader: Found', criticalImageUrls.size, 'critical images');
				}
				...
				function preloadCriticalImages() {
					...
					// Reduce to only 5 most critical images for faster loading
					var maxImagesToWait = Math.min(totalImages, 5); // Only wait for max 5 critical images
					
					// Convert Set to Array and limit to first 5
					var urlsArray = Array.from(criticalImageUrls).slice(0, 5);
					
					// Debug: Log how many images we're waiting for
					console.log('Preloader: Waiting for', urlsArray.length, 'critical images out of', totalImages, 'total');
					...
					for (var i = 0; i < urlsArray.length; i++) {
						(function(url) {
							...
							// Aggressive timeout: 300ms per image (very fast)
					...
					setTimeout(function() {
						if (!checkComplete) {
							checkComplete = true;
							imagesLoaded = true;
							console.log('Preloader: Force proceeding after timeout. Loaded:', loadedImages, 'Failed:', failedImages);
							tryHidePreloader();
						}
					}, 600); // Force proceed after 600ms (reduced from 800ms)
				}
		...
		to_top_init: function(self) {
			if ($.isFunction(jQuery.fn.UItoTop) && undefined !== window.fitspot.toTop) {
				$().UItoTop({
					text: fitspot.labels.totop_button,
					scrollSpeed: 600
				});
			}
		},
				...
				$termItem.on('click', function() {
					var slug = $(this).data('slug'),
						data = {
							...
							nonce: fitspot.new_smart_box_nonce,
							value_slug: slug,
		...
		subscribe_init: function(self) {
			CherryJsCore.variable.$document.on('click', '.subscribe-block__submit', function(event) {
				...
				var $this = $(this),
					...
					nonce = form.find('input[name="fitspot_subscribe"]').val(),
